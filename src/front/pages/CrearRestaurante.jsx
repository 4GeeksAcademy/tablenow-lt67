...
export const CrearRestaurante = () => {
    ...
    // Campos básicos (Los que ya tenías)
    ...
    // Campos nuevos de develop (IA y Categorización)
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            // Mantenemos tu alerta de éxito de develop
            alert("📍 Location captured successfully");
        },
        (error) => {
            setLoadingLocation(false);
            // Recuperamos la lógica de Hanna que es más útil para el usuario
            const msg = error.code === 1
                ? "Permiso denegado. Por favor, activa la ubicación en tu navegador."
    ...
    e.preventDefault();
    if (!store.tokenOwner) return alert("No hay sesión activa / No active session found.");
    if (!imageUrl) return alert("Por favor, sube una imagen / Please upload an image.");
    if (!latitud || !longitud) return alert("Por favor, captura la ubicación GPS / Please capture GPS location.");
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/restaurants`, {
                ...
                body: JSON.stringify({
                    ...
                    image_url: imageUrl,
                    // Aseguramos que sean números decimales (Lógica de Hanna)
                    latitud: parseFloat(latitud),
                    longitud: parseFloat(longitud),
                    // Mantenemos toda tu lógica de IA y Horarios
